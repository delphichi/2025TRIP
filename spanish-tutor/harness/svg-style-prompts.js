...
export function buildSvgPrompt(styleKey) {
    ...
    return [
        ...
        '',
        '## 輸出規則',
        '- **你的回應第一個字元必須是 `<`**（也就是 `<svg`）· 不要任何前言/開場白',
        '- **你的回應最後一個字元必須是 `>`**（也就是 `</svg>`）· 不要任何結尾/說明',
        '- 不要 markdown code fence · 不要 ```svg ``` 這種包裝',
        '- 若輸入是截圖含 UI 元素（狀態列 / logo / 留言數）· 忽略 UI · 只看漫畫主體',
        '- SVG 尺寸建議 viewBox="0 0 400 400"（若原圖直式可用 400×500）',
        '- 不使用 <script> / on* event 屬性 / 外部 xlink:href（會被 Harness 拒絕）',
        '- 保持路徑精簡 · 總長度控制在 15000 字元以內',
        '- **避免重複元素**：不要輸出兩個相同位置、相同尺寸的 shape · 每個元素都要有獨立意義',
        '- 座標保持在 viewBox 內',
    ].join('\n');
}
