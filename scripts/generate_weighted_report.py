...
針對兩種分數各做一次：
  1. SPMO score（12-1 動能 ÷ 年化波動）——這是真實驗證過落差的地方
  2. point（4W/13W/26W 綜合動能）——回答使用者的問題「V1 的分數加權後
     是不是也會不同」，用同一套加權邏輯套一次，不重新設計 V1 的排名公式
...
SCORE_DEFS = [
    ...
    ("point", "📊 Point（4W/13W/26W 綜合動能，V1 既有分數）",
     "V1 的 point 分數本身沒有市值變數，這裡套用同一套「先選股、再用分數×市值算權重」的邏輯，"
     "驗證使用者的問題：V1 的排名如果考慮市值，是不是也會不一樣。"),
]
  ...
  <div class="header">
    ...
    <div class="htitle">
      ...
      <p>9/6 用 2026-02-27 資料回測真實 SPMO 3 月調倉發現：我們的分數排名能抓到「誰該入選」（MU/GOOGL/JNJ
         ...
         權重 = 動能分數 × 市值，不是純分數排名。這份報表把兩種排法並排比較，SPMO 分數（原本就有）跟
         V1 的 point 分數（既有 daily-latest.html 用的動能分數）各做一次，回答「V1 加權後是不是也不同」。
